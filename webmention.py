...
@app.route(
    "/publish/<any(flickr,github,mastodon,meetup,twitter):silo>",
    methods=["GET", "HEAD"],
)
    ...
    * entity: the :class:`models.Publish` or :class:`models.Webmention` entity for
    ...
    def fetch_mf2(self, url, id=None, require_mf2=True, raise_errors=False):
        ...
        if require_mf2 and (not items or not items[0]):
            self.error(
                "No microformats2 data found in " + resp.url,
                data=mf2,
                html="""
...
"""
    ...
    def error(
        ...
        flask_util.error(
            str(resp), status=status, response=jsonify(resp), exc_info=log_exception
        )
    ...
    def report_error(self, resp, status=None):
        ...
        if (
            "Deadline exceeded while waiting for HTTP response" in resp
            or "urlfetch.Fetch() took too long" in resp
            or
            # WordPress Jetpack bugs
            # https://github.com/snarfed/bridgy/issues/161
            '"resp": "invalid_input"' in resp
            or
            # https://github.com/snarfed/bridgy/issues/750
            '"error": "jetpack_verification_failed"' in resp
            or
            ...
            "Invalid argument, 'thread': Unable to find thread" in resp
            or
            ...
            '"message":"Image file size must be' in resp
            or
            # Twitter media file number limits
            "Tweet with media must have exactly 1 gif or video" in resp
            or
            # Facebook image type/size req'ts
            "Missing or invalid image file" in resp
            or "Your photos couldn't be uploaded. Photos should be less than 4 MB"
            in resp
            or
            # Twitter duplicate publish attempts
            "Status is a duplicate." in resp
            or "You have already favorited this status." in resp
            or "You have already retweeted this" in resp
            or
            # Facebook duplicate publish attempts
            "This status update is identical to the last one you posted." in resp
            or
