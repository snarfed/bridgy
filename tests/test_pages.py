...
class PagesTest(testutil.AppTest):
    ...
    def test_poll_now_and_retry_response_missing_key(self):
        for endpoint in "/poll-now", "/retry":
            for body in {}, {
                "key": self.responses[0].key.urlsafe().decode()
            }:  # hasn't been stored
    ...
    def test_delete_source_callback(self):
        ...
        resp = self.client.get(
            "/delete/finish?"
            + "auth_entity="
            + self.sources[0].auth_entity.urlsafe().decode()
            + "&state="
            + encoded_state
        )
    ...
    def test_delete_source_declined(self):
        ...
        resp = self.client.get("/delete/finish?declined=True&state=" + encoded_state)

        self.assertEqual(302, resp.status_code)
        self.assertEqual(
            "http://withknown.com/bridgy_callback?"
            + urlencode([("result", "declined")]),
            resp.headers["Location"],
        )

    def test_delete_start_redirect_url_error(self):
        self.mox.StubOutWithMock(testutil.OAuthStart, "redirect_url")
        testutil.OAuthStart.redirect_url(state=mox.IgnoreArg()).AndRaise(
            tweepy.TweepError("Connection closed unexpectedly...")
        )
        self.mox.ReplayAll()
        ...
        self.assertEqual(
            ["FakeSource API error 504: Connection closed unexpectedly..."],
    ...
    def test_user_page_lookup_with_username_etc(self):
        ...
        for id in "FooBar", "Snoøpy Barrett", "foox.com":
            ...
            self.assertEqual(
                f"http://localhost/fake/{self.sources[0].key.id()}",
                resp.headers["Location"],
            )
    ...
    def test_social_user_page_mf2(self):
        ...
        self.assertEqual(["http://fa.ke/profile/url"], hcard["properties"].get("url"))
        self.assertEqual(["enabled"], hcard["properties"].get("bridgy-account-status"))
        self.assertEqual(["enabled"], hcard["properties"].get("bridgy-listen-status"))
        self.assertEqual(["enabled"], hcard["properties"].get("bridgy-publish-status"))
        ...
        for item, resp in zip(hcard["children"], expected_resps):
            ...
            self.assertEqual(
                [json_loads(resp.activities_json[0])["url"]],
    ...
    def test_user_page_private_twitter(self):
        ...
        self.assertNotIn(
            "most of your recent posts are private", resp.get_data(as_text=True)
        )
    ...
    def test_user_page_publish_url_with_unicode_char(self):
        ...
        Publish(parent=PublishedPage(id=url).key, source=self.sources[0].key).put()
    ...
    def test_user_page_escapes_html_chars(self):
        ...
        self.assertIn(
            '<span class="glyphicon glyphicon-transfer">', resp.get_data(as_text=True)
        )
    ...
    def test_users_page(self):
        ...
        for source in self.sources:
            self.assertIn(
                f'<a href="{source.bridgy_path()}" title="{source.label()}"',
                resp.get_data(as_text=True),
            )
        self.assertEqual(200, resp.status_code)
    ...
    def test_edit_web_sites_add(self):
        ...
        self.assertEqual(
            ['Added <a href="http://foo.com/">foo.com</a>.'], get_flashed_messages()
        )
    ...
    def test_edit_web_sites_add_existing(self):
        ...
        self.assertEqual(
            ['<a href="http://foo.com/">foo.com</a> already exists.'],
            get_flashed_messages(),
        )
    ...
    def test_edit_web_sites_delete(self):
        ...
        self.assertEqual(
            ['Removed <a href="https://bar">bar</a>.'], get_flashed_messages()
        )
    ...
    def test_edit_web_sites_delete_multiple_urls_same_domain(self):
        ...
        self.assertEqual(
            ['Removed <a href="https://foo.com/baz">foo.com/baz</a>.'],
            get_flashed_messages(),
        )
...
class DiscoverTest(testutil.AppTest):
    ...
    def check_discover(self, url, expected_message):
        ...
        detail = " ".join(
            (
                ...
                repr(location),
                repr(resp.get_data(as_text=True)),
            )
        )
    ...
    def test_discover_url_silo_event(self):
        ...
        self.check_discover(
            "http://fa.ke/events/123",
            "Discovering now. Refresh in a minute to see the results!",
        )

    def test_discover_url_silo_not_post_url(self):
        self.check_discover(
            "http://fa.ke/", "Sorry, that doesn't look like a FakeSource post URL."
        )

    def test_discover_twitter_profile_url_error(self):
        """https://console.cloud.google.com/errors/7553065641439031622"""
        auth_entity = TwitterAuth(
            id="foo", user_json="", token_key="", token_secret=""
        ).put()
        self.source = twitter.Twitter(
            id="foo", features=["listen"], auth_entity=auth_entity
        )
        self.source.put()
        self.check_discover(
            "https://twitter.com/bltavares",
