...
class ResponseTest(testutil.AppTest):
    ...
    def test_get_or_save_restart_existing_new_synd_url(self):
        ...
        SyndicatedPost(
            parent=source.key, original="http://or/ig", syndication=synd
        ).put()
        SyndicatedPost(
            parent=source.key, original=None, syndication=synd
        ).put()  # check that we don't die on blanks
    ...
    def test_get_or_save_activity_changed(self):
        ...
        self.assert_equals(
            [old_resp_json, json_dumps(new_resp_json)], response.old_response_jsons
        )
        ...
        self.assert_equals(
            [old_resp_json, json_dumps(new_resp_json), json_dumps(newer_resp_json)],
    ...
    def test_url(self):
        self.assertEqual(
            "http://localhost/fake/%s" % self.sources[0].key.string_id(),
    ...
    def test_get_type(self):
        self.assertEqual(
            "repost", Response.get_type({"objectType": "activity", "verb": "share"})
        )
...
class SourceTest(testutil.AppTest):
    ...
    def test_create_new_domain(self):
        ...
        for user_json in (
            ...
            if user_json is not None:
                auth_entity = testutil.FakeAuthEntity(
                    id="x", user_json=json_dumps(user_json)
                )
                auth_entity.put()
            with self.app.test_request_context():
        ...
        with self.app.test_request_context():
            source = FakeSource.create_new(auth_entity=auth_entity)
        self.assertEqual(
            ["http://foo.org/", "http://bar.com/", "http://baz/", "https://baj/biff"],
            source.domain_urls,
        )
    ...
    def test_create_new_rereads_domains(self):
        key = FakeSource.new(
            features=["listen"], domain_urls=["http://foo"], domains=["foo"]
        ).put()
    ...
    def test_create_new_merges_domains(self):
        ...
        with self.app.test_request_context():
            source = FakeSource.create_new(auth_entity=auth_entity)
        self.assertEqual(
            ["http://bar/", "http://baz/", "http://foo/"], source.domain_urls
        )
    ...
    def test_create_new_too_many_domains(self):
        ...
        auth_entity = testutil.FakeAuthEntity(
            id="x", user_json=json_dumps({"urls": [{"value": u} for u in urls]})
        )
    ...
    def test_create_new_domain_url_path_fails(self):
        auth_entity = testutil.FakeAuthEntity(
            id="x", user_json=json_dumps({"urls": [{"value": "http://flaky/foo"}]})
        )
...
class SyndicatedPostTest(testutil.AppTest):
    ...
    def test_insert_replaces_blanks(self):
        ...
        r = SyndicatedPost.insert(
            self.source, "http://silo/no-original", "http://original/newly-discovered"
        )
    ...
    def test_insert_auguments_existing(self):
        ...
        """
        r = SyndicatedPost.insert(
            self.source, "http://silo/post/url", "http://original/different/url"
        )
        self.assertIsNotNone(r)
    ...
    def test_get_or_insert_by_syndication_do_not_duplicate_blanks(self):
        ...
        SyndicatedPost.insert_syndication_blank(self.source, "http://silo/no-original")
