...
class SuperfeedrTest(testutil.AppTest):
    def setUp(self):
        ...
        self.source = testutil.FakeSource(
            id="foo.com", domains=["foo.com"], features=["webmention"]
        )
        self.source.put()
    ...
    def test_subscribe(self):
        ...
        self.expect_requests_post(
            superfeedr.PUSH_API_URL, feed, data=expected, auth=mox.IgnoreArg()
        )

        post_a = BlogPost(
            id="A", source=self.source.key, feed_item=item_a, unsent=["http://a.com/"]
        )
        post_b = BlogPost(
            id="B", source=self.source.key, feed_item=item_b, unsent=["http://b.com/"]
        )
    ...
    def test_handle_feed_allows_bridgy_publish_links(self):
        ...
        self.assert_equals(
            ["https://brid.gy/publish/twitter"], BlogPost.get_by_id("A").unsent
        )
    ...
    def test_handle_feed_unwraps_t_umblr_com_links(self):
        ...
        post = BlogPost(
            id="A", source=self.source.key, feed_item=item, unsent=["http://wrap/ped"]
        )
    ...
    def test_handle_feed_cleans_links(self):
        ...
        post = BlogPost(
            id="A", source=self.source.key, feed_item=item, unsent=["http://abc/"]
        )
        self.expect_task("propagate-blogpost", key=post)
        self.mox.ReplayAll()
    ...
    def test_notify_view(self):
        ...
        post = BlogPost(
            id="X", source=self.source.key, feed_item=item, unsent=["http://x/y"]
        )
        self.expect_task("propagate-blogpost", key=post)
        self.mox.ReplayAll()
    ...
    def test_handle_feed_truncates_links(self):
        ...
        item_a = {"permalinkUrl": "A", "content": "a http://a http://b http://c z"}
        post_a = BlogPost(
            id="A",
            source=self.source.key,
            feed_item=item_a,
