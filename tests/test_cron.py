...
class CronTest(testutil.BackgroundTest):
    ...
    def test_replace_poll_tasks(self):
        ...
        sources = [
            ...
            FakeSource.new(
                features=["listen"],
                created=month_ago,
                last_poll_attempt=day_and_half_ago,
            ).put(),
        ]

        self.expect_task(
            "poll", source_key=sources[4], last_polled="1970-01-01-00-00-00"
        )
        self.mox.ReplayAll()
    ...
    def test_update_twitter_pictures(self):
        ...
        for screen_name in ("a", "b", "c"):
            ...
            auth_entity.put()
            sources.append(
                Twitter.new(auth_entity=auth_entity, features=["listen"]).put()
            )
