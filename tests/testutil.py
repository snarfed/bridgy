...
class FakeGrSource(gr_source.Source):
    ...
    def clear(cls):
        cls.like = (
            cls.reaction
        ) = cls.share = cls.event = cls.rsvp = cls.etag = cls.last_search_query = None
    ...
    def get_activities_response(
        ...
        for activity in activities:
            ...
            obj["tags"] = [
                ...
                if "verb" not in tag
                or (tag["verb"] in ("like", "react") and fetch_likes)
                or (tag["verb"] == "share" and fetch_shares)
                or (tag["verb"] == "mention" and fetch_mentions)
            ]
    ...
    def scraped_to_activities(self, scraped, count=None, fetch_extras=False):
        activities = self.get_activities(
            count=count,
            fetch_replies=fetch_extras,
            fetch_likes=fetch_extras,
            fetch_shares=fetch_extras,
            fetch_mentions=fetch_extras,
        )
    ...
    def create(self, obj, include_link=gr_source.OMIT_LINK, ignore_formatting=False):
        verb = obj.get("verb")
        ...
        if "content" not in obj:
            return gr_source.creation_result(
                abort=False, error_plain="No content", error_html="No content"
            )
    ...
    def preview_create(
        self, obj, include_link=gr_source.OMIT_LINK, ignore_formatting=False
    ):
    ...
    def expect_task(self, queue, eta_seconds=None, **kwargs):
        ...
        def check_task(task):
            ...
            if eta_seconds is not None:
                got = util.to_utc_timestamp(
                    task.task.schedule_time
                ) - util.to_utc_timestamp(util.now_fn())
                delta = eta_seconds * 0.2 + 10
        ...
        return (
            tasks_client.create_task(mox.Func(check_task))
            .InAnyOrder()
            .AndReturn(Task(name="my task"))
        )
