        ...
        for resp in expected + stored:
            if "activities_json" not in ignore:
                resp.activities_json = [
                    json_dumps(json_loads(a), sort_keys=True)
                    for a in resp.activities_json
                ]
            if "response_json" not in ignore:
                resp.response_json = json_dumps(
                    json_loads(resp.response_json), sort_keys=True
                )

        self.assert_entities_equal(
            expected, stored, ignore=("created", "updated") + ignore
        )
...
class PollTest(TaskTest):
    ...
    def post_task(
        ...
        if expect_poll:
            ...
            self.expect_task(
                "poll",
                eta_seconds=expect_poll.total_seconds(),
    ...
    def test_poll_status_polling(self):
        ...
        self.expect_get_activities().WithSideEffects(check_poll_status).AndReturn(
            {"items": []}
        )
        self.mox.ReplayAll()
        self.post_task()
    ...
    def test_resolved_url(self):
        ...
        self.expect_requests_head(
            "http://will/redirect", redirected_url="http://final/url"
        )
    ...
    def test_resolve_url_fails(self):
        ...
        self.mox.ReplayAll()
        self.post_task()
        self.assert_equals(["http://fails/resolve"], self.responses[0].key.get().unsent)
    ...
    def test_invalid_and_blocklisted_urls(self):
        ...
        self.post_task()
        self.assert_equals(["http://tar.get/good"], self.responses[0].key.get().unsent)
    ...
    def test_strip_utm_query_params(self):
        ...
        self.post_task()
        self.assert_equals(["http://foo/bar?a=b"], self.responses[0].key.get().unsent)
    ...
    def test_strip_utm_query_params_after_redirect(self):
        ...
        self.expect_requests_head(
            "http://redir/0", redirected_url="http://first/?utm_medium=x"
        ).InAnyOrder()
        self.expect_requests_head(
            "http://redir/1", redirected_url="http://second/?utm_source=Twitter"
        ).InAnyOrder()
        self.mox.ReplayAll()
        self.post_task()
    ...
    def test_same_response_for_multiple_activities(self):
        ...
        resp = Response.query().get()
        self.assert_equals(
            ["tag:source.com,2013:%s" % id for id in ("a", "b", "c")],
    ...
    def test_search_for_links_skips_redirected_posse_post(self):
        ...
        self.expect_requests_head(
            "http://sho.rt/post", redirected_url="http://or.ig/post"
        )
        self.mox.ReplayAll()
        self.post_task()
        ...
        try:
            for err in (
                ...
                urllib.error.HTTPError(
                    ...
                    "foo",
                    {},
                    io.StringIO('{"meta":{"error_type":"OAuthAccessTokenException"}}'),
                ),
            ):
                self.mox.UnsetStubs()
                self.setUp()
    ...
    def test_rate_limiting_error(self):
        ...
        error_body = json_dumps(
            {
                "meta": {
                    "code": 429,
                    "error_message": "The maximum number of requests...",
        ...
        self.post_task(
            expect_poll=FakeSource.RATE_LIMITED_POLL, expect_last_polled=util.EPOCH
        )
        source = self.sources[0].key.get()
    ...
    def test_etag(self):
        ...
        self.expect_get_activities(etag='"my etag"', min_id="c").AndReturn(
            {"items": [], "etag": '"new etag"'}
        )
        self.mox.ReplayAll()
        self.post_task()
    ...
    def test_cache_trims_to_returned_activity_ids(self):
        ...
        self.post_task()
        self.assert_equals(
            {"prefix b": 0}, json_loads(source.key.get().last_activities_cache_json)
        )

    def test_slow_poll_never_sent_webmention(self):
        self.sources[0].created = NOW - (
            FakeSource.FAST_POLL_GRACE_PERIOD + datetime.timedelta(minutes=1)
        )
        self.sources[0].put()
    ...
    def test_do_not_refetch_hfeed(self):
        ...
        self.sources[0].last_hfeed_refetch = hour_ago = NOW - datetime.timedelta(
            hours=1
        )
        self.sources[0].put()
    ...
    def test_response_changed(self):
        ...
        self.assert_equals(
            replies, json_loads(source.key.get().seen_responses_cache_json)
        )
        ...
        self.post_task(reset=True, expect_poll=FakeSource.FAST_POLL)
        self.assert_equals(
            [r.key for r in self.responses[:4]],
...
class DiscoverTest(TaskTest):
    ...
    def test_restart_existing_tasks(self):
        ...
        for resp in Response.query():
            ...
            self.assert_equals(
                ["http://target1/post/url", "http://target/2"], resp.unsent, resp.key
            )
    ...
    def _test_get_activities_error(self, status):
        self.expect_get_activities(
            activity_id="b", user_id=self.sources[0].key.id()
...
class PropagateTest(TaskTest):
    ...
    def expect_webmention(
        ...
        **kwargs,
        ...
        if source_url is None:
            source_url = (
                "http://localhost/comment/fake/%s/a/1_2_a"
        ...
        if discover:
            ...
            call = self.expect_requests_get(
                target, html, headers=headers, status_code=discover_status, **kwargs
            ).InAnyOrder()
        ...
        if send or (send is None and endpoint):
            call = self.expect_requests_post(
                endpoint,
                data={
                    "source": source_url,
                    "target": target,
    ...
    def test_propagate_from_error(self):
        ...
        self.post_task()
        self.assert_response_is(
            "complete", NOW + LEASE_LENGTH, sent=["http://target1/post/url"]
        )
        self.assert_equals(NOW, self.sources[0].key.get().last_webmention_sent)
    ...
    def test_non_html_url(self):
        """Target URLs that aren't HTML should be ignored."""
        self.expect_requests_head(
            "http://target1/post/url", content_type="application/mpeg"
        )
        self.mox.ReplayAll()
        self.post_task()
    ...
    def test_webmention_post_accept_header(self):
        ...
        self.expect_requests_post(
            ...
            timeout=999,
            data={
                "source": "http://localhost/comment/twitter/rhiaro/a/1_2_a",
                "target": "http://target1/post/url",
            },
            allow_redirects=False,
            headers={"Accept": "*/*"},
    ...
    def test_unicode_in_target_url(self):
        ...
        """
        url = "https://maps/?q=" + urllib.parse.quote_plus(
            "3 Cours de la République".encode()
        )
    ...
    def test_leased(self):
        ...
        self.post_task(expected_status=ERROR_HTTP_RETURN_CODE)
        self.assert_response_is(
            "processing", leased_until, unsent=["http://target1/post/url"]
        )
    ...
    def test_webmention_fail_and_succeed(self):
        ...
        self.post_task(expected_status=ERROR_HTTP_RETURN_CODE)
        self.assert_response_is(
            "error", None, error=["http://first"], sent=["http://second"]
        )
        self.assert_equals(NOW, self.sources[0].key.get().last_webmention_sent)
    ...
    def test_webmention_exception(self):
        ...
        self.post_task(expected_status=ERROR_HTTP_RETURN_CODE)
        self.assert_response_is(
            "error", None, error=["http://error"], sent=["http://good"]
        )
        self.assert_equals(NOW, self.sources[0].key.get().last_webmention_sent)
    ...
    def test_translate_appspot_to_brid_gy(self):
        ...
        self.responses[0].put()
        source_url = (
            "https://brid.gy/comment/fake/%s/a/1_2_a" % self.sources[0].key.string_id()
        )
    ...
    def test_activity_id_not_tag_uri(self):
        ...
        source_url = (
            "https://brid.gy/comment/fake/%s/AAA/1_2_a"
            % self.sources[0].key.string_id()
        )
        self.expect_webmention(source_url=source_url, target="http://good")
    ...
    def test_response_with_multiple_activities(self):
        ...
        source_url = (
            "https://brid.gy/comment/fake/%s/%%s/1_2_a"
            % self.sources[0].key.string_id()
        )
    ...
    def test_propagate_blogpost(self):
        ...
        self.assert_response_is(
            "complete", NOW + LEASE_LENGTH, sent=["http://ok/two"], response=blogpost
        )
    ...
    def test_propagate_blogpost_allows_bridgy_publish_links(self):
        ...
        blogpost = models.BlogPost(
            id="http://x", source=source_key, unsent=["https://brid.gy/publish/twitter"]
        )
        blogpost.put()
        ...
        super().post_task(params={"key": blogpost.key.urlsafe().decode()})
        self.assert_response_is(
            "complete", response=blogpost, sent=["https://brid.gy/publish/twitter"]
        )

    def test_propagate_blogpost_follows_redirects_before_checking_self_link(self):
        source_key = FakeSource.new(domains=["fake"]).put()
        blogpost = models.BlogPost(
            id="http://x", source=source_key, unsent=["http://will/redirect"]
        )
        blogpost.put()

        self.expect_requests_head(
            "http://will/redirect", redirected_url="http://www.fake/self/link"
        )
        self.mox.ReplayAll()
    ...
    def test_post_response(self):
        ...
        self.responses[0].response_json = json_dumps(
            json_loads(self.responses[0].activities_json[0])
        )
        self.responses[0].put()
...
class PropagateBlogPostTest(TaskTest):
    ...
    def post_task(self, **kwargs):
        super().post_task(
            params={"key": self.blogposts[0].key.urlsafe().decode()}, **kwargs
        )
