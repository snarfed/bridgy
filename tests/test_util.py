...
class UtilTest(testutil.AppTest):
    ...
    def test_maybe_add_or_delete_source(self):
        auth_entity = FakeAuthEntity(
            id="x",
            user_json=json_dumps({"url": "http://foo.com/", "name": UNICODE_STR}),
        )
        auth_entity.put()
    ...
    def test_maybe_add_or_delete_without_web_site_redirects_to_edit_websites(self):
        for bad_url in None, "not>a<url", "http://fa.ke/xyz":
            ...
            self.assertEqual(302, rr.exception.code)
            self.assert_equals(
                f"http://localhost/edit-websites?source_key={key}", rr.exception.new_url
            )
    ...
    def test_add_to_logins_cookie(self):
        with app.test_request_context(headers={"Cookie": "logins=/other/1?bob"}):
            ...
            self.assertIn(
                cookie.format(id=id), rr.exception.get_response().headers["Set-Cookie"]
            )
    ...
    def test_get_webmention_target_blocklisted_urls(self):
        for resolve in True, False:
            self.assertTrue(
                util.get_webmention_target("http://good.com/a", resolve=resolve)[2]
            )
            for bad in (
                "http://facebook.com/x",
                ...
                self.assertFalse(
                    util.get_webmention_target(bad, resolve=resolve)[2], bad
                )

    def test_get_webmention_cleans_redirected_urls(self):
        self.expect_requests_head(
            "http://foo/bar", redirected_url="http://final?utm_source=x"
        )
        self.mox.ReplayAll()

        self.assert_equals(
            ("http://final", "final", True),
            util.get_webmention_target("http://foo/bar", resolve=True),
        )
        self.assert_equals(
            ("http://foo/bar", "foo", True),
    ...
    def test_get_webmention_target_too_big(self):
        ...
        self.mox.ReplayAll()
        self.assert_equals(
            ("http://orig", "orig", False), util.get_webmention_target("http://orig")
        )

    def test_get_webmention_target_not_http_https(self):
        self.assert_equals(
            ("chrome://flags", "flags", False),
    ...
    def test_requests_get_content_length_not_int(self):
        self.expect_requests_get(
            "http://foo/bar", "xyz", response_headers={"Content-Length": "foo"}
        )
        self.mox.ReplayAll()
    ...
    def test_no_accept_header(self):
        self.assertEqual(
            util.REQUEST_HEADERS, util.request_headers(url="http://foo/bar")
        )
        self.assertEqual(
            util.REQUEST_HEADERS, util.request_headers(source=Twitter(id="not-rhiaro"))
        )
    ...
    def test_rhiaro_accept_header(self):
        ...
        self.expect_requests_get(
            "http://rhiaro.co.uk/", "", headers=util.REQUEST_HEADERS_CONNEG
        )
        self.mox.ReplayAll()
...
class RegistrationCallbackTest(testutil.AppTest):
    class FakeAdd(oauth_views.Callback):
        ...
        def dispatch_request(self):
            util.maybe_add_or_delete_source(
                FakeSource, self.auth_entity, request.values.get("state")
            )
            return ""
    ...
    def setUp(self):
        ...
        self.FakeAdd.auth_entity = FakeAuthEntity(
    ...
    def test_user_url(self):
        ...
        self.assert_equals(["listen"], source.features)
        self.assert_equals(
            ["https://kylewm.com/", "http://fakeuser.com/"], source.domain_urls
        )
    ...
    def test_decline(self):
        ...
        self.assert_equals(302, resp.status_code)
        self.assert_equals(
            "http://withknown.com/bridgy_callback?result=declined",
            resp.headers["Location"],
        )
