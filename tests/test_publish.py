...
class FakeSend(publish.Send):
    ...
    def dispatch_request(self):
        state = util.encode_oauth_state(self.oauth_state) if self.oauth_state else None
...
class PublishTest(testutil.AppTest):
    def setUp(self):
        ...
        self.auth_entity = FakeSend.auth_entity = testutil.FakeAuthEntity(
            id="0123456789"
        )
        ...
        self.post_html = (
            '<article class="h-entry"><p class="e-content">%s</p></article>'
        )
        self.backlink = '\n<a href="http://localhost/publish/fake"></a>'

    def get_response(
        self, source=None, target=None, preview=False, interactive=False, params=None
    ):
        if params is None:
            params = {}
        params.update(
    ...
    def assert_response(self, expected, status=None, preview=False, **kwargs):
        ...
        self.assertEqual(
            status, resp.status_code, "%s != %s: %s" % (status, resp.status_code, body)
        )
        if preview:
            self.assertIn(expected, body, "%r\n\n=== vs ===\n\n%r" % (expected, body))
        else:
    ...
    def test_interactive_from_wrong_user_page(self):
        ...
        self.assertEqual(302, resp.status_code)
        self.assertEqual(
            f"http://localhost/fake/{other_source.id()}", resp.headers["Location"]
        )
        self.assertEqual(
    ...
    def test_already_published(self):
        ...
        completed = list(
            Publish.query(Publish.status == "complete", Publish.type == "post")
        )
        ...
        self.assertEqual(
            json_loads(created.get_data(as_text=True)),
    ...
    def test_already_published_interactive(self):
        ...
        self.assertIn(
            "Sorry, you've already published that page", get_flashed_messages()[0]
        )
    ...
    def test_publish_entity_too_much_contention(self):
        self.mox.StubOutWithMock(
            publish.PublishBase, "_get_or_add_publish_entity", use_mock_anything=True
        )
        ...
        publish.PublishBase._get_or_add_publish_entity("http://foo.com/bar").AndRaise(
            GrpcError()
        )
    ...
    def test_source_url_redirects(self):
        self.expect_requests_head(
            "http://will/redirect", redirected_url="http://foo.com/1"
        )
...
<a rel="shortlink" href="http://foo.com/short"></a>""",
    ...
    def test_no_content(self):
        self.expect_requests_get(
            "http://foo.com/bar", '<article class="h-entry"></article>'
        )
        self.mox.ReplayAll()
    ...
    def test_no_content_ignore_formatting(self):
        ...
        self.assert_error(
            "Could not find content", params={"bridgy_ignore_formatting": ""}
        )
        self.assertEqual("failed", Publish.query().get().status)
    ...
    def test_unpublishable_type(self):
        html = (
            '<p class="h-breadcrumb"><span class="e-content">not publishable</span></p>\n'
            + self.post_html % "foo"
        )
        self.expect_requests_get("http://foo.com/bar", html)
        self.mox.ReplayAll()
...
<article class="h-entry"><a class="u-like-of" href="xyz">W</a></article>""",
    ...
    def test_source_url_is_domain_url(self):
        ...
        self.mox.ReplayAll()
        self.assert_created(
            "foo - http://foo.com/?p=123", source="http://foo.com/?p=123"
        )

    def test_source_url_redirects_to_domain_url(self):
        self.expect_requests_head(
            "http://will/redirect", redirected_url="http://foo.com"
        )
        self.mox.ReplayAll()
        self.source.put()
        self.assert_error(
            "Looks like that's your home page.", source="http://will/redirect"
        )
...
""",
    ...
    def test_tumblr_backlink_in_t_umblr_com_url(self):
        ...
        self.expect_requests_get(
            "http://foo.com/bar", self.post_html % "foo", backlink=link
        )
        self.mox.ReplayAll()
...
<p class="e-content">foo</p></article>"""
    ...
    def test_report_error(self):
        ...
        self.mox.StubOutWithMock(
            error_reporting_client, "report", use_mock_anything=True
        )
        for subject in "Webmention None failed", "Preview preview new":
            error_reporting_client.report(
                subject,
                http_context=mox.IgnoreArg(),
                user="http://localhost/fake/foo.com",
            )
        ...
        self.mox.StubOutWithMock(
            self.source.gr_source, "preview_create", use_mock_anything=True
        )
    ...
    def test_silo_500_returns_502(self):
        ...
        err = requests.HTTPError(
            response=util.Struct(status_code="500", text="foooey bar")
        )
        self.source.gr_source.create(
            mox.IgnoreArg(),
            include_link=gr_source.INCLUDE_LINK,
...
<a class="u-bridgy-omit-link" href=""></a>
        ...
        resp = self.assert_success(
            "preview of foo - http://foo.com/bar",
            preview=True,
...
<a class="u-bridgy-ignore-formatting" href=""></a>
    ...
    def test_bridgy_content_query_param_unsupported(self):
        ...
        self.assert_error(
            "bridgy_fake_content parameter is not supported", params=params
        )
        self.assert_error(
            "bridgy_fake_content parameter is not supported",
            preview=True,
            params=params,
        )
...
<div class="p-bridgy-fake-content">use this</div>
        ...
        params = {"bridgy_omit_link": "false", "bridgy_ignore_formatting": "true"}
        self.assert_success(
            "use this - http://foo.com/bar", preview=True, params=params
        )
...
Join us!"""
        ...
        self.assertEqual(expected, json_loads(resp.get_data(as_text=True))["content"])
    ...
    def test_unicode(self):
        ...
        self.assert_created(
            text, preview=False, source=url, params={"bridgy_omit_link": ""}
        )
        self.assert_success(
            text, preview=True, source=url, params={"bridgy_omit_link": ""}
        )
...
"""
    ...
    def test_missing_backlink(self):
        ...
        super().expect_requests_get("http://foo.com/bar", self.post_html % "foo")
        self.mox.ReplayAll()
    ...
    def test_dont_escape_period_in_content(self):
        ...
        """
        self.expect_requests_get(
            "http://foo.com/bar", self.post_html % "<span /> 2016. #"
        )
        self.mox.ReplayAll()
...
""",
        ...
        self.assertNotIn("with images", resp.get_data(as_text=True))
...
""",
        ...
        self.assertEqual(
            ...
            json_loads(resp.get_data(as_text=True))["images"],
        )

        resp = self.assert_success("blah - http://foo.com/bar", preview=True)
        self.assertIn(
            "with images http://example.com/real", resp.get_data(as_text=True)
        )
...
""",
        ...
        self.mox.ReplayAll()
        self.assert_created(
            "Doug (@murderofcro.ws) is SOOPER excited about #pelikanhubs2017"
        )
...
"""
        ...
        self.mox.ReplayAll()
        resp = self.assert_created(
            "foo - http://foo.com/bar#baz", source="http://foo.com/bar#baz"
        )
...
""",
        ...
        self.mox.ReplayAll()
        self.assert_error(
            "Got fragment baz but no element found with that id.",
...
""",
    ...
    def test_delete_not_published_error(self):
        ...
        self.mox.ReplayAll()
        self.assert_error(
            "Can't delete this post from FakeSource because Bridgy Publish didn't originally POSSE it there"
        )
    ...
    def test_preview_delete_unsupported_silo(self):
        ...
        self.expect_requests_get("http://foo.com/bar", status_code=410)
        self.mox.StubOutWithMock(
            self.source.gr_source, "preview_delete", use_mock_anything=True
        )
        self.source.gr_source.preview_delete(mox.IgnoreArg()).AndRaise(
            NotImplementedError()
        )
        self.mox.ReplayAll()

        self.assert_error(
            "Sorry, deleting isn't supported for FakeSource yet", preview=True
        )
