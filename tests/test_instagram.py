...
class InstagramTest(testutil.AppTest):
    ...
    def test_canonicalize_url(self):
        ...
        for url in (
            ...
            self.assertEqual(
                "https://www.instagram.com/p/abcd/", self.source.canonicalize_url(url)
            )
    ...
    def test_get_activities_response_no_activity_id(self):
        Activity(
            id="tag:instagram.com,2013:123",
            source=self.source.key,
            activity_json=json_dumps({"foo": "bar"}),
        ).put()
        Activity(
            id="tag:instagram.com,2013:456",
            source=self.source.key,
    ...
    def test_get_comment_no_matching_id(self):
        self.assertIsNone(
            self.source.get_comment("333", activity=HTML_VIDEO_ACTIVITY_FULL)
        )
    ...
    def test_homepage_bad_html(self):
        ...
        self.assertIn(
            "Couldn't determine logged in Instagram user",
    ...
    def test_post(self):
        ...
        self.assertEqual(
            HTML_VIDEO_ACTIVITY_FULL, json_loads(activities[0].activity_json)
        )
    ...
    def test_post_merge_comments(self):
        ...
        activity_key = Activity(
            ...
            activity_json=json_dumps(existing_activity),
        ).put()
    ...
    def test_likes(self):
        ...
        resp = self.get_response(
            f"likes?id=tag:instagram.com,2013:123_456&{self.auth}",
    ...
    def test_poll(self):
        key = self.source.put()
        self.expect_task(
            "poll", eta_seconds=0, source_key=key, last_polled="1970-01-01-00-00-00"
        )
        self.mox.ReplayAll()
