...
class HandlersTest(testutil.AppTest):
    ...
    def check_response(self, url_template, expected_body=None, expected_status=200):
        resp = self.client.get(
            url_template % self.source.key.string_id(),
        ...
        if expected_body:
            ...
            actual = "\n".join(
                resp.get_data(as_text=True).splitlines()[header_lines:-1]
            )
...
""",
    ...
    def test_pass_through_source_errors(self):
        ...
        err = urllib.error.HTTPError(
            "url", 410, "Gone", {}, io.StringIO("Gone baby gone")
        )
        self.mox.StubOutWithMock(testutil.FakeSource, "get_activities")
        testutil.FakeSource.get_activities(activity_id="000", user_id=user_id).AndRaise(
            err
        )
        self.mox.ReplayAll()
    ...
    def test_handle_disable_source(self):
        ...
        self.assertIn(
            "Bridgy's access to your account has expired",
...
"""
    ...
    def test_comment_optimized_comments_activity_has_replies(self):
        ...
        FakeSource.get_comment(
            "a1",
            activity_id="000",
            activity_author_id=self.source.key_id(),
            activity=self.activities[0],
        ).AndReturn(replies["items"][0])
        self.mox.ReplayAll()
...
""",
    ...
    def test_repost_with_syndicated_post_and_mentions(self):
        ...
        models.SyndicatedPost(
            ...
            syndication="http://example.com/original/post",
        ).put()
...
""",
    ...
    def test_original_post_urls_follow_redirects(self):
        ...
        self.expect_requests_head(
            "http://or.ig/post", redirected_url="http://or.ig/post/redirect"
        ).InAnyOrder()
        self.expect_requests_head(
            "http://other/link", redirected_url="http://other/link/redirect"
        ).InAnyOrder()
        self.mox.ReplayAll()

        self.check_response(
            "/comment/fake/%s/000/111",
            """\
<article class="h-entry">
...
""",
    ...
    def test_cache(self):
        ...
        self.assert_multiline_equals(
            orig.get_data(as_text=True), cached.get_data(as_text=True)
        )
