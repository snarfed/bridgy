...
URL_BLOCKLIST = frozenset(("http://www.evdemon.org/2015/learning-more-about-quill",))
...
def add_poll_task(source, now=False):
    ...
    if now:
        ...
        if source.AUTO_POLL:
            ...
            eta_seconds += int(
                source.poll_period().total_seconds() * random.uniform(0.8, 1.2)
            )
...
def add_discover_task(source, post_id, type=None):
    ...
    add_task(
        "discover", source_key=source.key.urlsafe().decode(), post_id=post_id, type=type
    )
...
def report_error(msg, **kwargs):
    ...
    try:
        ...
        if not DEBUG:
            logging.warning(
                "Failed to report error to StackDriver! %s %s",
                msg,
                kwargs,
                exc_info=True,
            )
...
def request_headers(url=None, source=None):
    if (
        url
        and util.domain_from_link(url) in CONNEG_DOMAINS
        or source
        and source.bridgy_path() in CONNEG_PATHS
    ):
...
def replace_test_domains_with_localhost(url):
    ...
    if url and LOCAL:
        for test_domain, local_domain in LOCALHOST_TEST_DOMAINS:
            url = re.sub("https?://" + test_domain, "http://" + local_domain, url)
    return url
...
def host_url(path_query=None):
    ...
    base = (
        HOST_URL
        if util.domain_or_parent_in(domain, OTHER_DOMAINS)
...
def construct_state_param_for_add(state=None, **kwargs):
    ...
    if not state_obj:
        state_obj = {
            ...
            for field in ("callback", "feature", "id", "user_url")
        }
...
def unwrap_t_umblr_com(url):
    ...
    return (
        urllib.parse.parse_qs(parsed.query).get("z", [""])[0]
