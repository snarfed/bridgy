...
import twitter
from twitter import TwitterComment, TwitterPage, TwitterService
import models
import tasks_test
...
class TwitterPageTest(testutil.ModelsTest):

  def setUp(self):
    super(TwitterPageTest, self).setUp()
    ...
    self.page = TwitterPage(key_name='2468',
  ...
  def _test_new(self):
    ...
    got = TwitterPage.new(http, self.handler)
    self.assert_entities_equal(self.page, got, ignore=['created'])
    self.assert_entities_equal([self.page], TwitterPage.all(), ignore=['created'])
