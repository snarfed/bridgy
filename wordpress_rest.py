...
API_CREATE_COMMENT_URL = (
    "https://public-api.wordpress.com/rest/v1/sites/%s/posts/%d/replies/new?pretty=true"
)
API_POST_SLUG_URL = (
    "https://public-api.wordpress.com/rest/v1/sites/%s/posts/slug:%s?pretty=true"
)
...
class WordPress(models.Source):
    ...
    def new(auth_entity=None, **kwargs):
        ...
        avatar = (
            json_loads(auth_entity.user_json).get("avatar_URL")
        ...
        try:
            ...
            try:
                ...
                if (code == "400" and parsed.get("error") == "invalid_input") or (
                    code == "403"
                    and parsed.get("message") == "Comments on this post are closed"
                ):
    ...
    def get_site_info(cls, auth_entity):
        ...
        try:
            ...
            if code == "403" and '"API calls to this blog have been disabled."' in body:
                flash(
                    f'You need to <a href="http://jetpack.me/support/json-api/">enable the Jetpack JSON API</a> in {util.pretty_link(auth_entity.blog_url)}\'s WordPress admin console.'
                )
                redirect("/")
                return None
            raise
    ...
    def finish(self, auth_entity, state=None):
        if auth_entity:
            ...
            if site_info is None:
                ...
                logging.info(
                    "This is a self-hosted WordPress blog! %s %s",
                    auth_entity.key_id(),
                    auth_entity.blog_id,
                )
                return render_template(
                    "confirm_self_hosted_wordpress.html",
